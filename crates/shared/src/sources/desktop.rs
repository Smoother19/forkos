...
fn parse_desktop_file(path: &Path) -> Option<DesktopApp> {
    ...
    let name = fields.get("Name[fr]").or_else(|| fields.get("Name"))?.clone();
    ...
    let is_flatpak = path.to_str().map(|s| s.contains("flatpak")).unwrap_or(false);

    Some(DesktopApp { name, description, exec: clean_exec(&exec), is_flatpak })
}
