...
/// Lit ~/.local/share/recently-used.xbel et retourne les fichiers récents.
...
fn parse_xbel(xml: &str) -> Vec<RecentFile> {
    ...
    // Extrait chaque bloc <bookmark ...>
    let mut pos = 0;
    while let Some(start) = xml[pos..].find("<bookmark") {
        let abs_start = pos + start;
        // Trouve la fin de la balise ouvrante
    ...
    // Les plus récents en premier, limite à 30
    results.reverse();
    results.truncate(30);
    results
}
...
fn parse_bookmark_tag(tag: &str) -> Option<RecentFile> {
    ...
    // Décoder l'URI file:/// → chemin absolu
    ...
    // Ignore les chemins non-existants
...
/// Extrait la valeur d'un attribut XML : attr="valeur"
...
/// Décode le percent-encoding d'une URL (%20 → espace, etc.)
...
/// Convertit une date ISO 8601 en temps relatif lisible
fn format_relative_time(iso: &str) -> String {
    // Format : 2024-01-15T14:32:00Z
    // On extrait juste la date pour simplifier
    let date_part = iso.split('T').next().unwrap_or("");
    if date_part.is_empty() {
        return "récemment".to_string();
    }

    // Comparaison simple avec la date du jour via le timestamp de modification fichier
    // Pour l'instant on retourne juste la date formatée
